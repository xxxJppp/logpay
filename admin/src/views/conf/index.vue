<template>
  <div class="wrapper">
    ...
    <div class="phone">
        ...
        <el-select style="margin-left:46px;width:188px;" v-if="codeEdit" v-model="value" placeholder="请选择">
        <el-option
        ...
        <div class="code" v-if="codeEdit">
            <div class="crumbs" style="height:30px;line-height:30px;">
                <el-breadcrumb separator="/">
                    <el-breadcrumb-item style="margin:5px 0;font-size:20px;">手机 {{ value }} 收款配置</el-breadcrumb-item>
                    <el-switch
                ...
                <div>
                    <el-button type="primary" style="margin-left:178px;" @click="limitMoney()" size="mini">设置限额(默认无限额)</el-button>
                    <el-button type="primary" style="margin-left:10px;" @click="remarkAccount()" size="mini">设置备注(默认无备注)</el-button>
                </div>
                ...
                <el-dialog title="设置备注" :visible.sync="remarkEdit" :width="dialogWidth" top="200px">
                <el-form :model="remarkForm">
                    <el-form-item label="支付宝" :label-width="formLabelWidth">
                        <el-input v-model="remarkForm.alipayRemark" autocomplete="off" placeholder="18888888888"></el-input>
                    </el-form-item>
                    <el-form-item label="微信" :label-width="formLabelWidth">
                        <el-input v-model="remarkForm.wxpayRemark" autocomplete="off" placeholder="18888888888"></el-input>
                    </el-form-item>
                </el-form>
                <div slot="footer" class="dialog-footer">
                    <el-button @click="remarkEdit = false">取 消</el-button>
            ...
            <div class="container">
                <div class="attention">
                    <p style="color:#67c23a;font-size:18px;">第1步.用收款手机 {{ value }} 的支付宝扫描二维码下方获得userid，并在下方配置(*不使用支付宝可跳过)。</p>
                ...
                <p style="color:#67c23a;font-size:18px;">第2步.上传手机 {{ value }} 的微信支付宝二维码(至少要上传支付宝微信的任意码即不固定码 <a style="color:blue;" target="_blank" href="https://www.logpay.cn/document/#getCode">怎么获取收款二维码？</a>)。</p>
                <el-upload
                    :multiple = "true"
                    class="upload-demo"
                    drag
                    ...
                    <div class="el-upload__tip" slot="tip">单次最多上传300张<span style="color:red;">(*可批量多选上传，如需更大量上传请联系客服，上传成功后将在二维码管理列表出现)</span></div>
                </el-upload>
                <div class="codeManage" style="margin:1% 0;">
                <div class="crumbs">
                    <el-breadcrumb separator="/">
                        <el-breadcrumb-item style="margin:1% 0;font-size:16px;">手机 {{ value }} 二维码管理</el-breadcrumb-item>
                    </el-breadcrumb>
                </div>
                    <template>
                ...
                <p style="color:#67c23a;font-size:18px;">第3步. <el-button type="success" style="margin:10px 0;" @click="handleTest()">线上测试</el-button></p>
                <el-dialog title="线上测试" :visible.sync="testEdit" :width="dialogWidth" top="100px">
                    <el-form :model="testForm">
                        ...
                        <el-form-item label="异步回调地址" :label-width="formLabelWidth">
                            <el-input v-model="testForm.notifyUrl" autocomplete="off" placeholder="支付完通知服务器的地址 必填"></el-input>
                        </el-form-item>
                        <el-form-item label="同步回调地址" :label-width="formLabelWidth">
                            <el-input v-model="testForm.returnUrl" autocomplete="off" placeholder="支付完跳转的地址 可不填"></el-input>
                        </el-form-item>
                    </el-form>
                    <div slot="footer" class="dialog-footer">
  ...
  data() {
    ...
    methods:{
        ...
        async getRemark() {
            ...
            this.remarkForm.alipayRemark = res.data.alipayRemark
            this.remarkForm.wxpayRemark = res.data.wxpayRemark
        },
        // 提交修改
        async submitRemark() {
            let data = {
                uid:this.uid,
                phoneId:this.value,
                alipayRemark:this.remarkForm.alipayRemark,
                wxpayRemark:this.remarkForm.wxpayRemark
            }
        ...
        async handleAddPhone () {
            if (this.phoneIds.length >= 1) {
                if (this.meal == 'mf' || this.meal == 'bz') {
                    return this.$message.success('使用多手机需升级高级版')
                }
            }
            let data = {
                uid:this.uid
        ...
        async getPhoneIds () {
            ...
            res.data.forEach(v=>{
                if (v.alipayRemark && v.wxpayRemark) {
                    list.push({value:v.id,label:'手机'+ v.id + ' 支付宝: ' + v.alipayRemark + ' 微信: ' + v.wxpayRemark})
                } else {
                    if (!v.alipayRemark) {
                        v.alipayRemark = '无备注'
                    }
