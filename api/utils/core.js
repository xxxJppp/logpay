...
let { uid, price, orderNumber, pid, payType, notifyUrl, returnUrl, sign, orderUid, orderName, ip, format} = remoteData
//获取html页面
...
//						let alipayCodeUrl = 'alipays://platformapi/startapp?appId=20000123&actionType=scan&biz_data={"s":"money","u":"'+ phone.userid +'","a":"'+payPrice+'","m":""}'
                ...
                let h5Url = 'https://render.alipay.com/p/s/i?scheme=' + encodeUrl( 'alipays://platformapi/startapp?saId=10000007&qrcode=' + encodeUrl('https://api.logpay.cn/server/api/alipayH5?u='+ phone.userid + '&a='+ payPrice+'&_s=web-other'))
                    ...
                    process.send('render','alipay.html',{
                            ...
                            .then( qrcode =>{
                                if (!qrcode) {
                                    Qrcode.findOne({price:'不固定金额', uid, phoneId, type:'支付宝'})
                                        ...
                                        .catch(err => process.send('json','配置错误!请您配置支付宝不固定额收款码!'))
                                } else {
                                    if ( format && format == 'json' ) {
                                        process.send('json',{
                                            code: 1,
                ...
                .catch( err =>process.send('json','ali-qr-find-error'))
                }
            })
            .catch( err=> process.send('json','ali-uid-find-error'))
        }
    if (payType === 'wxpay') {
        Qrcode.findOne({price:payPrice, uid, phoneId, type:'微信'})
                        ...
                        Qrcode.findOne({price:'不固定金额', uid, phoneId, type:'微信'})
                            .then( code =>{
                                if ( format && format == 'json' ) {
                                    ...
                                    process.send('render','wxpay.html',{
                                    createTime:createTime,
                                    money: '请手动输入 '+payPrice,
                                    price: price,
                                    payPrice: payPrice,
                            ...
                            .catch(err => process.send('json','配置错误!请您配置微信不固定额收款码!'))
                    } else {
                        if ( format && format == 'json' ) {
                            process.send('json',{
                                code: 1,
                ...
                .catch( err =>process.send('json','wx-qr-find-error'))
    }
    if (payType === 'lakala') {
        Qrcode.findOne({price:'不固定金额', uid, phoneId, type:'拉卡拉'})
            .then( code =>{
                ...
                if ( format && format == 'json' ) {
                    ...
                    process.send('render','lakala.html',{
                    createTime:createTime,
                    money: '请手动输入 '+payPrice,
                    price: price,
                    payPrice: payPrice,
            ...
            .catch(err => process.send('json','配置错误!请您配置拉卡拉不固定额收款码!'))
    }
}
// 判断手机还是电脑
// let agentID = ''
...
// }
if (!uid) {
    process.send('json','缺少uid参数')
    try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }
}
if (!price) {
    process.send('json','缺少price参数')
    try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }
}
if (!payType) {
    process.send('json','缺少payType参数')
    try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }
}
if (!orderNumber) {
    process.send('json','缺少orderNumber参数')
    try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }
}
if (!notifyUrl) {
    process.send('json','缺少notifyUrl参数')
    try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }
}
// 获取指定用户的token
...
// 免签
let o = await orderNumberFind( orderNumber, uid, orderUid, payType, price, payPrice ).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) } })
if (o) {
    if (o.status === 2) {
    process.send('json','订单已支付!')
    try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }
    }
    if (o.status === 1) {
        process.send('json','回调通知失败,请联系客服!')
        try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }      
    }
    if (o.status === -1) {
        if (o.expire === 0) {
            process.send('json','订单已过期!,请重新发起支付!')
            try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }      
            ...
            Order.deleteOne({orderNumber:o.orderNumber})
                 .then(order=>{ process.send('json','系统繁忙!') try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }})
                 .catch( err=>{ process.send('json','300处理失败!') try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }})
        }
    }
}
if (!o) {
       let existOrder = await orderFind(uid, orderUid, ip, payType, price).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e)}})
       if (!existOrder) {
           //套餐的fee计算
                if(uid !== '10001') {
                    let merchant = await getUserMeal(uid).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e)}})
                    if (merchant.meal == 'mf') {
                        fee = await getMfFee(price).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e)}})
                    } else if (merchant.meal == 'bz') {
                        fee = await getPayFee('bz', merchant, uid, price).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e)}})
                    } else if (merchant.meal == 'gj') {
                        fee = await getPayFee('gj', merchant, uid, price).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e)}})
                    } else {
                        fee = await getOtherFee(merchant, price).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e)}})
                    }
                    // 判断商户余额是否足够
                    if (parseFloat(merchant.money)-parseFloat(fee) < 0) {
                        return process.send('json','商户余额不足,请及时冲值!')
                    }
                    } else {
                        fee = '0.00'
                    }
                    ...
                    if ( sign === signs ) {
                    // 价格排序算法
                    let sortOrder = await priceSort(uid, price, payType, orderNumber).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) } })
                    ...
                    if(isNaN(payPrice) || payPrice <= 0) {
                        return process.send('json',"请稍后再试")
                    }
                    // 获取启用手机数组[1,2,3]
                    let phoneArr = await getPhoneArr(uid).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }})
                    ...
                    let successOrder = await orderSave(order).catch(err => { process.send('json',err) try { process.kill( pid,'SIGTERM') } catch(e) { console.log(e) }})
                    if (successOrder) {
                        childTimer.send({
                            orderNumber,
                            expire,
