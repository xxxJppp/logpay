...
// 照片文字识别
const client = require('../../utils/baiduOcr')
const request = require('request')
const qiniu = require('qiniu')
const passport = require('passport')

// 上传千牛
router.post('/qrcode/updata', passport.authenticate("jwt",{session:false}), async (req, res) => {
...
router.post('/qrcode/add', passport.authenticate("jwt",{session:false}), async (req, res) => {
    try {
...
router.get('/qrcode/all', passport.authenticate("jwt",{session:false}), async (req, res) => {
    try {
...
router.delete('/qrcode/del', passport.authenticate("jwt",{session:false}), async (req, res) => {
    try{
        let params = req.body
...
// 删除指定支付方式
router.post('/qrcode/removeType', passport.authenticate("jwt",{session:false}), (req, res)=>{
...
// 删除全部
router.post('/qrcode/removeAll', passport.authenticate("jwt",{session:false}), (req, res)=>{
...
// 删除选中
router.post('/qrcode/removeSelect', passport.authenticate("jwt",{session:false}), (req, res)=>{
...
// 禁用 启用二维码
router.post('/qrcode/codeOpen', passport.authenticate("jwt",{session:false}), (req, res)=>{
...
router.get('/qrcode/image',async (req, res)=>{
	...
	let codeImage = qr_image.image(imageText,{type: 'png',margin:2})
	codeImage.pipe(res)
})
...
router.get('/qrcode/codeBase64', async(req, res)=>{
	...
	let imageDataBase64 = 'data:image/png;base64,' + await imgData('https://api.logpay.cn/qrcode/image?text=' + req.query.text)
